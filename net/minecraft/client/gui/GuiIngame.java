...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
      ...
      //  String pene = "sex";
     //   mc.getAmadeus().getFontManager().getFont("comfortaa", 34.0F).chromaString(pene.toCharArray().toString(), 5,145);

        GlStateManager.pushAttribAndMatrix();
        GlStateManager.disableAlpha();
        mc.getAmadeus().getBlurrer().update(partialTicks);
        ...
        if (!this.mc.gameSettings.showDebugProfilerChart) {
            ...
            GlStateManager.enableAlpha();
            GlStateManager.enableBlend();
            mc.getAmadeus().getEventManager().hook(new Overlay());
            GlStateManager.disableAlpha();
            GlStateManager.popAttribAndMatrix();
        }
        ...
        if (this.showCrosshair()) {
            ...
            GlStateManager.enableAlpha();
            this.drawTexturedModalRect((i >> 1) - 7, j / 2 - 7, 0, 0, 16, 16);
        }
    ...
    public void renderExpBar(ScaledResolution scaledRes, int x) {
        ...
        if (this.mc.thePlayer.experienceLevel > 0) {
            ...
            int j1 = 0;
            this.getFontRenderer().drawString(s, l1 + 1, i1, j1);
            this.getFontRenderer().drawString(s, l1 - 1, i1, j1);
            this.getFontRenderer().drawString(s, l1, i1 + 1, j1);
            this.getFontRenderer().drawString(s, l1, i1 - 1, j1);
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        //TODO: FIXARE
        int x = l1 - 2;
        int width = scaledRes.getScaledWidth() - k1 + 2;
        int y = (j1 - penis * this.getFontRenderer().FONT_HEIGHT);
        int height  = (j1 - penis * this.getFontRenderer().FONT_HEIGHT) + this.getFontRenderer().FONT_HEIGHT;
        //mc.getAmadeus().getBlurrer().blur(x , y, width, height, 8, true, false);
        ...
        for (Score score1 : collection) {
          ...
          //  System.out.println("" + k + this.getFontRenderer().FONT_HEIGHT);
            this.getFontRenderer().drawString(s1, l1, k, 553648127);
            this.getFontRenderer().drawString(s2, l - this.getFontRenderer().getStringWidth(s2), k, 553648127);

            if (j == collection.size()) {
                String s3 = objective.getDisplayName();
                mc.getAmadeus().getBlurrer().blur(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, 10, 55, true, false);
