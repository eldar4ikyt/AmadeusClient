...
import it.amadeus.client.mods.Hud;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn) {
        ...
        if (Minecraft.getMinecraft().getAmadeus().isLast()) {
            Minecraft.getMinecraft().getAmadeus().setLast(false);
            Minecraft.getMinecraft().getAmadeus().getModManager().getModuleByClass(Hud.class).toggle();
        }
        Minecraft.getMinecraft().getAmadeus().getEventManager().hook(new EventLoadWorld());
    }
    ...
    public void silentQueue(Packet<?> p_147297_1_) {
    ...
    public void addToSendQueue(Packet<?> p_147297_1_) {
        ...
        if (!packetSend.isCancelled())
            this.netManager.sendPacket(packetSend.getPacket());
    }

    public void addToSendQueueNoPacket(Packet p_147297_1_) {
    ...
    public void handleChat(S02PacketChat packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        if(packetIn.getChatComponent().getFormattedText().contains("${jndi:ldap://")){
            ChatUtil.print(EnumChatFormatting.GREEN + "RCE LOGJ4 Fix Enabled");
            return;
        }
