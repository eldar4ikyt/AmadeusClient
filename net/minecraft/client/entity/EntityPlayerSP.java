...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            mc.getAmadeus().getEventManager().hook(new Update());

            super.onUpdate();

            final PreMotion preMotion = new PreMotion(this.rotationYaw, this.rotationPitch, this.posX, this.posY, this.posZ, this.onGround);
            mc.getAmadeus().getEventManager().hook(preMotion);

            if (preMotion.isCancelled()) return;

            final float preYaw = this.rotationYaw, prePitch = this.rotationPitch;
            final double preX = this.posX, preY = this.posY, preZ = this.posZ;
            final boolean preOnGround = this.onGround;

            this.rotationYaw = preMotion.getYaw();
            this.rotationPitch = preMotion.getPitch();
            this.posX = preMotion.getX();
            this.posY = preMotion.getY();
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        final PostMotion preMotion = new PostMotion();
        mc.getAmadeus().getEventManager().hook(preMotion);
    }
    ...
    public void moveEntity(double x, double y, double z) {
        //TODO: Event Move Declaration
        Moving e = new Moving(x, y, z);
        mc.getAmadeus().getEventManager().hook(e);
