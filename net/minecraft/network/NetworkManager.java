...
import it.amadeus.client.viamcp.handler.VRDecodeHandler;
import it.amadeus.client.viamcp.handler.VREncodeHandler;
import it.amadeus.client.viamcp.utils.NettyUtil;
...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            try {
                PacketReceive packetReceive = new PacketReceive(p_channelRead0_2_);
                Minecraft.getMinecraft().getAmadeus().getEventManager().hook(packetReceive);
                if (!packetReceive.isCancelled()) p_channelRead0_2_.processPacket(this.packetListener);
    ...
    public void dispatchPacket(final Packet<?> inPacket, final GenericFutureListener<? extends Future<? super Void>>[] futureListeners) {
        if (GuiMultiplayer.isBungeeHack() && inPacket instanceof C00Handshake && ((C00Handshake)inPacket).getRequestedState() == EnumConnectionState.LOGIN) {
            ((C00Handshake)inPacket).setIp(((C00Handshake)inPacket).getIp() + "\000" + "0.0.0.0" + "\000" + UUID.nameUUIDFromBytes(("OfflinePlayer:" + Minecraft.getMinecraft().getSession().getUsername()).getBytes()).toString().replace("-", ""));
        }
    ...
    public void sendPacketNoEvent(Packet packetIn) {
        if (this.channel != null && this.channel.isOpen()) {
            ...
            this.outboundPacketsQueue.add(
                    new InboundHandlerTuplePacketListener(packetIn, null));
        }
    }
