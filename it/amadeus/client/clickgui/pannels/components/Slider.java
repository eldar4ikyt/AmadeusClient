...
public class Slider extends Component {
    ...
    public int drawScreen(int mouseX, int mouseY, int x, int y) {
        ...
        if (this.dragging) {
            float toSet = ((float) mouseX - (float) this.x) / (float) this.parent.getWidth();
            if (toSet > 1) {
                toSet = 1;
            }
            if (toSet < 0) {
            ...
            if (this.set.getValue() instanceof Double) {
                this.set.setValueObject(toSet2);
            } else if (this.set.getValue() instanceof Integer) {
        ...
        Gui.drawRect(this.x, this.y + ClickGui.getFont().getStringHeight(name) + 6, (int) (this.x + (this.parent.getWidth() * distance)), this.y + this.height - 3, ClickGui.getPrimaryColor().getRGB());
        Gui.drawRect((int) (this.x + (this.parent.getWidth() * distance)), this.y + ClickGui.getFont().getStringHeight(name) + 6, (int) (this.x + (this.parent.getWidth() * distance)) + 5 > parent.getWidth() ? (int) (this.x + (this.parent.getWidth() * distance)) : (int) (this.x + (this.parent.getWidth() * distance)) + 5, this.y + this.height - 3, new Color(255, 255, 255).getRGB());
        GlStateManager.pushMatrix();
        float scale = 1;
        GlStateManager.scale(scale, scale, scale);
        ClickGui.getFont().drawStringWithFont(name, (this.x + 2) / scale, (y + 3) / scale, ClickGui.getPrimaryColor().getRGB());
